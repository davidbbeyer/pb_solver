############################################################################################################
# Check that the numerical procedure used for the paper gives the correct result in the limit
# of vanishing Bjerrum length.
############################################################################################################
...
def calculate_charges(pKa, pH_range):
    ...
    alphas_HH = np.asarray([alpha_henderson_hasselbalch(pH, pKa) for pH in pH_range])
    alphas_HH_Donnan = np.asarray([alpha_henderson_hasselbalch_donnan(pH, pKa, r_colloid, phi_colloid, n_surface_groups) for pH in pH_range])
...
pKa_range = np.linspace(1.0, 10.0, 10)
